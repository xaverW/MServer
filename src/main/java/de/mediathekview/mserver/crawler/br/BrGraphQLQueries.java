...
public class BrGraphQLQueries {
  ...
  public static String getQuery2GetAllClipIds(
      ZonedDateTime minimumAvailableUntil, int clipCount, String cursor) {
    ...
    StringVariable gtTodayVariable =
        new StringVariable("gte", minimumAvailableUntil.format(DATE_FORMATTER));
    RecursiveAbstractVariable videoAvailableVariable =
        new RecursiveAbstractVariable("availableUntil", gtTodayVariable);
  ...
  private static String addBroadcastInterface() {
    return addObjectConstruct(
        "broadcasts(first: 1, orderBy: START_ASC)",
        addObjectConstruct("edges", addObjectConstruct("node", "__typename", "start", "id")));
  }
